(function ($, window, document) {
    ...
    var pluginName = "dctimepicker",
        defaults = {
            now: today.getHours() + ':' + today.getMinutes(),
            twentyFour: false,
            upArrow: 'dctimepicker__controls__control-up',
            downArrow: 'dctimepicker__controls__control-down',
            close: 'dctimepicker__close',
     ...
     */
    function dctimepicker(element, options) {
        this.element = $(element);
        this.options = $.extend({}, defaults, options);

        this.element.addClass('hasdctimepicker');
        this.element.attr('onkeypress', 'return false;');
        this.element.attr('aria-showingpicker', 'false');
        this.createPicker();
        this.timepicker = $('.dctimepicker');
        this.up = $('.' + this.options.upArrow);
        this.down = $('.' + this.options.downArrow);
        this.separator = $('.dctimepicker__controls__control--separator');
        this.hoursElem = $('.dctimepicker__controls__control--hours');
        this.minutesElem = $('.dctimepicker__controls__control--minutes');
        this.secondsElem = $('.dctimepicker__controls__control--seconds');
        this.meridiemElem = $('.dctimepicker__controls__control--meridiem');
    ...
    $.extend(dctimepicker.prototype, {
        ...
        createPicker: function () {
            if ($('.dctimepicker').length === 0) {
                var picker = '<div class="dctimepicker"><p class="dctimepicker__title">' + this.options.title + '<span class="dctimepicker__close"></span></p><ul class="dctimepicker__controls"><li class="dctimepicker__controls__control"><span class="' + this.options.upArrow + '"></span><span class="dctimepicker__controls__control--hours" tabindex="-1">00</span><span class="' + this.options.downArrow + '"></span></li><li class="dctimepicker__controls__control--separator"><span class="dctimepicker__controls__control--separator-inner">:</span></li><li class="dctimepicker__controls__control"><span class="' + this.options.upArrow + '"></span><span class="dctimepicker__controls__control--minutes" tabindex="-1">00</span><span class="' + this.options.downArrow + '"></span></li>';
                if (this.options.showSeconds) {
                    picker += '<li class="dctimepicker__controls__control--separator"><span class="dctimepicker__controls__control--separator-inner">:</span></li><li class="dctimepicker__controls__control"><span class="' + this.options.upArrow + '"></span><span class="dctimepicker__controls__control--seconds" tabindex="-1">00</span><span class="' + this.options.downArrow + '"></span> </li>';
                }
                picker += '<li class="dctimepicker__controls__control"><span class="' + this.options.upArrow + '"></span><span class="dctimepicker__controls__control--meridiem" tabindex="-1">AM</span><span class="' + this.options.downArrow + '"></span></li></ul></div>';
                $('body').append(picker);
        ...
        attach: function (element) {
            ...
            /* Handle click events for closing dctimepicker */
            var clickHandler = function (event) {
                ...
                if ($(self.timepicker).is(':visible')) {
                    ...
                    } else if ($(event.target).closest(self.timepicker).length || $(event.target).closest($('.hasdctimepicker')).length) { //Clicking the dctimepicker or one of it's inputs
        ...
        attachKeyboardEvents: function () {
            $(document).on('keydown', $.proxy(function (event) {
                switch (event.keyCode) {
                    case 9:
                        if (event.target.className !== 'hasdctimepicker') {
            ...
            $(this.up).add(this.down).off('mousedown click touchstart').on('mousedown click', {
                ...
                if (event.type == 'mousedown') {
                    timeOut = setInterval($.proxy(function (args) {
                        args.dctimepicker.changeValue(operator, args.input, this);
                    }, this, {'dctimepicker': passedData.dctimepicker, 'input': passedData.input}), 200);
                } else {
                    passedData.dctimepicker.changeValue(operator, passedData.input, this);
                }
        ...
        makePickerInputClearable: function(input) {
            ...
            $('[data-clear-picker]').on('click', function(event) {
               $(this).siblings('.hasdctimepicker').val('');
            });
        },
    ...
    $.fn[pluginName] = function (options, index) {
        if (!$.isFunction(dctimepicker.prototype['_' + options])) {
            return this.each(function () {
                if (!$.data(this, "plugin_" + pluginName)) {
                    $.data(this, "plugin_" + pluginName, new dctimepicker(this, options));
                }
            });
        }
        else if ($(this).hasClass('hasdctimepicker')) {
            if (index !== undefined) {
