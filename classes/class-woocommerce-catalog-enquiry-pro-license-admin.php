...
if ( ! class_exists( 'Woocommerce_Catalog_Enquiry_Pro_License_Admin' ) ) {
    class Woocommerce_Catalog_Enquiry_Pro_License_Admin {
    	public $data_key                          = '';
    	public $product_id       = '';
    	...
    	public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '' ) {
    		...
    		if ( is_admin() ) {
    			...
    			$this->wc_am_licence_menu_slug			 = 'mvx-catalog-license-admin';
    			$this->text_domain                       = esc_attr( $text_domain );
                $this->wc_am_licence_menu_title			 = esc_html__( 'License', $this->text_domain );
        ...
        public function register_menu() {
            add_submenu_page( 'catalog', __( $this->wc_am_licence_menu_title, $this->text_domain ), __( $this->wc_am_licence_menu_title, $this->text_domain ), 'manage_woocommerce', $this->wc_am_licence_menu_slug, array( $this, 'load_settings_panel' ) );
        }
        ...
        public function load_settings_panel() {
			...
			<div class="wrap mvx-settings-wrap">
				...
				?>
				<form class='mvx_vendors_settings mvx_subtab_content mvx_<?php echo $tab; ?>_<?php echo $tab_section; ?>_settings_group' method="post" action="options.php">
					<?php
					if ( ! empty( $tab_desc[$tab] ) ) {
						echo '<h4 class="mvx-tab-description">' . $tab_desc[$tab] . '</h4>';
					}
					?>
					<?php
        ...
        public function wc_am_licence_tabs() {
			...
			$sublinks = array();
			// unset multivendorx pro clint tab
			unset($this->tabs_list['wc_am_client_143434_dashboard']);
			foreach ( $this->tabs_list as $tab_id => $tab ) {
				...
				foreach ( $this->get_licence_subtabs( $tab_id ) as $subtab_id => $subtab ) {
					...
					if ( $subtab_id === $current_section ) {
						$sublinks[] = "<li><a class='current mvx_sub_sction' href='?page=" . $this->wc_am_licence_menu_slug . "&tab=$tab_id&tab_section=$subtab_id'>$sublink</a></li>";
					} else {
						$sublinks[] = "<li><a class='mvx_sub_sction' href='?page=" . $this->wc_am_licence_menu_slug . "&tab=$tab_id&tab_section=$subtab_id'>$sublink</a></li>";
					}
				}
			}
			...
			echo '<div class="mvx_subtab_container">';
			echo '<ul class="subsubsub mvxsubtabadmin">';
		...
		*/
		...
		public function get_tabs_list() {
			return apply_filters('mvx_license_tabs_list', array());
		}
